...
function DashboardInsightsTabs({
  ...
  const allReturnsZero =
    ...
    performance.every(
      (item) =>
        Math.abs(Number(item.profitLossPercent || 0)) < 0.01
    );

  const nextDividend = [...dividendCalendar]
    .filter(
     (item) => new Date(item.expectedDate) >= new Date()
    )
    .sort(
     (a, b) =>
  ...
  return (
    <section className="mb-8 rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl">
      <div className="mb-5">
        <h2 className="text-xl font-bold">Dashboard Insights</h2>

        <p className="mt-1 text-sm text-slate-400">
          Review portfolio actions, assistant recommendations, insights, strengths and health score.
        </p>
      </div>
